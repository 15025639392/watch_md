...
7. Watch 显示路线卡片：名称、距离、同步时间、“使用此路线开始”按钮。
8. 路线安装成功只代表本地可用；创建绑定路线的会话前必须经过 Watch 端用户确认。
...
10. Watch 没有可用计划路线时允许进入自由记录模式：可以开始、暂停、继续、二次确认结束和生成摘要，但不写路线投影字段，不生成偏航或转向事件。
11. 自由记录中途收到路线时，实现数据判断层和确认层：先计算当前位置到路线的最近投影点和距离，距离可接受且用户确认后，才从该投影点开始后续路线进度、偏航和转向计算；确认前历史轨迹不补算。
...
6. 无计划路线时 Watch 能完成自由记录闭环，`nearestRouteDistanceMeters` 和 `routeProgressMeters` 为空。
7. 自由记录中途接入路线必须经过距离判断和用户确认，不能静默切换当前会话。
...
5. 回到路线后退出偏航并记录事件。
6. 无计划路线时地图显示自由记录状态，不显示计划路线、起终点、剩余距离和偏航状态。
7. 定位不稳时不频繁报偏航。
8. 接近明显转向点时提醒。
9. 地图底图失败时仍能显示路线线框、当前位置和已走轨迹。
10. Watch 地图页没有图层切换入口；iPhone 可承担更完整的地图图层检查。
11. 中途接入路线后，偏航检测从用户确认时刻的最近路线投影点开始，不补算确认前轨迹。
...
11. 实现最小 Watch 路线卡片占位：能显示一条本地/mock 路线的名称、距离、同步时间和“使用此路线开始”按钮；按钮可以先进入占位地图或占位会话页。
