...
当 Watch 已经在自由记录中，iPhone 后续下发路线并且用户确认使用该路线导航时，偏航检测从确认时刻开始。

规则：

1. 不对确认前的历史轨迹补算路线投影、路线进度或偏航事件。
2. 使用确认时刻的当前可信定位点计算最近路线投影点。
3. 将该投影点作为后续 `routeProgressMeters` 的初始锚点。
4. 后续定位点优先在该锚点附近窗口匹配，再按正常路线跟随逻辑扩大窗口。
5. 如果当前定位点到路线距离超过中途接入阈值，不启动偏航状态机，只提示路线离当前位置较远。
