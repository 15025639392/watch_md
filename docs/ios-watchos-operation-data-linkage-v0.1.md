...
当前已落代码的 UI、按钮、临时实现和目标规格边界，参见 [当前 UI 与操作逻辑对齐说明 v0.1](./current-ui-operation-alignment-v0.1.md)。后续修改页面结构或操作入口时，应同步维护该文档，避免把 mock 同步、自由记录占位路线或尚未实装的复盘能力误解为最终产品规则。
...
1. iPhone 已有 Watch 中枢、回传详情和实际轨迹复盘入口。
2. 当前复盘地图稳定展示 Watch 回传的实际轨迹，并用实际轨迹首尾点作为起终点。
3. 计划路线叠加、偏航段高亮、偏航事件列表和 GPX 导出仍是后续实现项，不能按当前代码视为已完成。
